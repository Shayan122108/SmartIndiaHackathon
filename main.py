...
try:
    ...
    SMS_AVAILABLE = True
except ImportError as _sms_err:
    import logging as _log
    _log.warning(f"SMS tools unavailable: {_sms_err}")
    SMS_AVAILABLE = False
    subscriber_registry = None

try:
    from tools.gov_alert_tools import poll_government_alerts, GovAlertPoller
    GOV_ALERTS_AVAILABLE = True
except ImportError as _gov_err:
    import logging as _log2
    _log2.warning(f"Gov alert tools unavailable: {_gov_err}")
    GOV_ALERTS_AVAILABLE = False
    poll_government_alerts = None
...
# Global government alerts cache
GOV_ALERTS_CACHE: list = []
GOV_ALERTS_LAST_FETCHED: str = ""
...
async def poll_gov_alerts_loop():
    """Background task: poll government/WHO/NDMA RSS feeds every 30 minutes."""
    global GOV_ALERTS_CACHE, GOV_ALERTS_LAST_FETCHED
    if not GOV_ALERTS_AVAILABLE:
        logger.warning("Gov alert polling skipped — tools unavailable")
        return
    while True:
        try:
            logger.info("Polling government outbreak alert feeds...")
            alerts = poll_government_alerts(force_refresh=True)
            GOV_ALERTS_CACHE = alerts
            GOV_ALERTS_LAST_FETCHED = datetime.now().isoformat()
            logger.info(f"Gov alert cache updated: {len(alerts)} alerts")
        except Exception as e:
            logger.error(f"Error polling gov alerts: {e}")
...
async def lifespan(app: FastAPI):
    ...
    gov_alert_task = asyncio.create_task(poll_gov_alerts_loop())
    
    try:
        yield
    finally:
        logger.info("Shutting down AI Health Agent API...")
        for task in (cleanup_task, gov_alert_task):
            task.cancel()
            try:
                await task
...
async def run_emergency_simulation(simulation_type: str):
    ...
    try:
        ...
        if alerts_generated:
            ...
            # Auto-broadcast via SMS to all subscribers
            if SMS_AVAILABLE and subscriber_registry:
                try:
                    sms_result = broadcast_sms_alert(alert, registry=subscriber_registry)
                    logger.info(
                        f"Simulation SMS broadcast: {sms_result['total_sent']} sent, "
                        f"{sms_result['total_failed']} failed"
                    )
                    response_data["sms_broadcast"] = {
                        "total_sent": sms_result["total_sent"],
                        "total_failed": sms_result["total_failed"],
                    }
                except Exception as sms_err:
                    logger.error(f"Simulation SMS broadcast error: {sms_err}")
        ...
        if result.get('generated_alert'):
            ...
            if SMS_AVAILABLE and subscriber_registry:
                try:
                    sms_result = broadcast_sms_alert(alert, registry=subscriber_registry)
                    logger.info(
                        f"SMS broadcast: {sms_result['total_sent']} sent, "
                        f"{sms_result['total_failed']} failed"
                    )
                    response_data["sms_broadcast"] = {
                        "total_sent": sms_result["total_sent"],
                    ...
                    logger.error(f"SMS broadcast error: {sms_err}")
...
class SMSRequest(BaseModel):
    to_number: str = Field(..., description="E.164 phone number, e.g. +919148033536")
    message: str = Field(..., min_length=1, max_length=1600, description="SMS body")


class SubscribeRequest(BaseModel):
    phone_number: str = Field(..., description="E.164 phone number to subscribe")
    name: str = Field(default="Subscriber", description="Friendly name for the subscriber")


@app.post("/sms/send", tags=["SMS"])
async def send_sms_endpoint(req: SMSRequest):
    """Send a one-off SMS to a specific number."""
    if not SMS_AVAILABLE:
        raise HTTPException(status_code=503, detail="SMS service not available")
    result = send_sms(req.to_number, req.message)
    if not result["success"]:
        raise HTTPException(status_code=502, detail=result.get("error", "SMS send failed"))
    return result


@app.post("/sms/subscribe", tags=["SMS"])
async def sms_subscribe(req: SubscribeRequest):
    """Subscribe a phone number to receive emergency SMS alerts."""
    if not SMS_AVAILABLE:
        raise HTTPException(status_code=503, detail="SMS service not available")
    sub = subscriber_registry.add(req.phone_number, name=req.name)
    return {
        "message": f"Subscribed {sub.phone_number} successfully",
        "subscriber": {
        ...
        "total_active": subscriber_registry.count_active(),
    }


@app.delete("/sms/unsubscribe", tags=["SMS"])
async def sms_unsubscribe(phone_number: str):
    """Unsubscribe a phone number from SMS alerts."""
    if not SMS_AVAILABLE:
        raise HTTPException(status_code=503, detail="SMS service not available")
    found = subscriber_registry.remove(phone_number)
    if not found:
        raise HTTPException(status_code=404, detail="Phone number not found in subscriber list")
    return {
        "message": f"{phone_number} unsubscribed",
        "total_active": subscriber_registry.count_active(),
    }


@app.get("/sms/subscribers", tags=["SMS"])
async def list_sms_subscribers():
    """List all SMS subscribers (active and inactive)."""
    if not SMS_AVAILABLE:
        raise HTTPException(status_code=503, detail="SMS service not available")
    return {
        "total_active": subscriber_registry.count_active(),
        "total": len(subscriber_registry.all()),
        "subscribers": subscriber_registry.to_dict_list(),
    }
...
@app.get("/gov-alerts/cached", tags=["Government Alerts"])
async def get_cached_gov_alerts():
    """Return the most recent government alert cache (updated every 30 min automatically)."""
    return {
        "count": len(GOV_ALERTS_CACHE),
        "last_fetched": GOV_ALERTS_LAST_FETCHED or "Not fetched yet",
        "alerts": GOV_ALERTS_CACHE,
        "available": GOV_ALERTS_AVAILABLE,
    }


@app.post("/gov-alerts/fetch", tags=["Government Alerts"])
async def fetch_gov_alerts_now():
    """Manually trigger a fresh poll of all government alert feeds right now."""
    global GOV_ALERTS_CACHE, GOV_ALERTS_LAST_FETCHED
    if not GOV_ALERTS_AVAILABLE:
        raise HTTPException(status_code=503, detail="Government alert tools not available")
    try:
        alerts = poll_government_alerts(force_refresh=True)
        GOV_ALERTS_CACHE = alerts
        GOV_ALERTS_LAST_FETCHED = datetime.now().isoformat()
        return {
            "count": len(alerts),
            "last_fetched": GOV_ALERTS_LAST_FETCHED,
            "alerts": alerts,
        }
    except Exception as e:
        logger.error(f"Manual gov alert fetch failed: {e}")
