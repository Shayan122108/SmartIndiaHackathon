...
def _get_whatsapp_url():
    """Build the WhatsApp messages API URL from env var."""
    phone_number_id = os.getenv("WHATSAPP_PHONE_NUMBER_ID")
    if not phone_number_id:
        return None, {"error": "WHATSAPP_PHONE_NUMBER_ID not set in .env. Get it from Meta Developer Console → WhatsApp → API Setup."}
    return f"https://graph.facebook.com/v22.0/{phone_number_id}/messages", None
